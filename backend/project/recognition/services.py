...
class UserService:
    ...
    def authenticate_user(image_sent: str, limite_similaridade: float = 0.6):
        ...
        for user in users:
            for image_user in user.imagens.all():
                image_user_array = face_recognition.load_image_file(
                    image_user.image.path
                )

                try:
                    image_user_encoding = face_recognition.face_encodings(
                        image_user_array
                    )[0]
                except IndexError:
                    continue
                ...
                distance = face_recognition.face_distance(
                    [image_user_encoding], image_sent_encoding
                )[0]
    ...
    def upload_image_user(user_id, image):
        ...
        image_user = models.ImageUser(
            user=user, image=os.path.join("users", original_name)
        )
